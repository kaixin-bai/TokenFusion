...
class ModifiedSegFormerHead(nn.Module):
    def __init__(self, feature_strides=None, in_channels=128, embedding_dim=256, num_classes=20, **kwargs):
        ...
        self.ca = ChannelAttention(in_channels=256)
    ...
    def forward(self, s_features, m_features):
        ...
        x_s = __transform(s_features)
        x_m = __transform(m_features)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, s, m):
        # Using s_features to calculate the attention weights
        avg_out = self.fc(self.avg_pool(s))
        max_out = self.fc(self.max_pool(s))
        out = avg_out + max_out
        weights = self.sigmoid(out)
        # Apply weights to m_features
        return weights * m  # 用single计算通道注意力权重，确保m_features是主导
